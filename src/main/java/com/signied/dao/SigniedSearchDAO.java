...
//				System.out.println("검색한 room list : " + vo);
	...
	public List<RoomVO> detailSearchRoom(String sort, List<String> viewTypes, List<String> roomTypes) throws SQLException {
...
/*	         뷰타입 조건 추가
	        if (viewTypes != null && !viewTypes.isEmpty()) {
	            query.append(" AND view_type IN (");
	            for (int i = 0; i < viewTypes.size(); i++) {
	                query.append("?");
	                if (i < viewTypes.size() - 1) query.append(", ");
	            }
	            query.append(")");
	        }
	        ...
	        if (roomTypes != null && !roomTypes.isEmpty()) {
	            ...
	            for (int i = 0; i < roomTypes.size(); i++) {
	                query.append("?");
	                if (i < roomTypes.size() - 1) query.append(", ");
	            }
	            query.append(")");
	        }
	        ...
	        if (viewTypes != null) {
	            for (String viewType : viewTypes) {
	                ps.setString(paramIndex++, viewType);
	            }
	        }
	        if (roomTypes != null) {
	            for (String bedType : roomTypes) {
	                ps.setString(paramIndex++, bedType);
	            }
	        }
	    ...
	    public static void main(String[] args) {
	        try (Connection conn =  여기에 DB 연결 코드 ) {
	            HotelQueryService service = new HotelQueryService(conn);
	            ...
	            ResultSet rs = service.getHotels("낮은 가격순으로 정렬", viewTypes, bedTypes);
