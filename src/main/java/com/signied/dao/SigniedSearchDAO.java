...
//				reservation 테이블에서 주어진 체크인과 체크아웃 날짜 조건을 만족하는 데이터를 그룹화하여 해당 기간에 예약된 방의 개수(daily_reserved_count)를 계산
//				계산된 정보를 room 테이블과 결합(LEFT JOIN)
//				만약 room 테이블의 특정 방에 해당하는 예약 정보가 reservation 테이블에 없으면 daily_reserved_count는 NULL 값으로 처리
				+ "LEFT JOIN (\n"
...
//				결과를 방의 가격(roomPrice)에 따라 오름차순으로 정렬
	...
	public List<RoomVO> detailSearchRoom(String sort, List<String> viewTypes, List<String> roomTypes, List<RoomVO> roomLists)
			throws SQLException {
