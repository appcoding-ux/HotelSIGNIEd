...
$(function() {
	$('.amounts').on('click', function() {
		var amount = $('#adultAmount').val();
		var amount2 = $('#childAmount').val();
		$.ajax({
			url: 'HotelServlet?command=people_num', // 서버 엔드포인트 URL을 입력
			type: 'get',
			data: { amountdata: amount, childAmount: amount2 },
			success: function(response) {
				var date3 = response.amountView;
				var date4 = response.amountView2;
				console.log(date3);
				console.log(date4);
				// 서버에서 반환된 응답을 hotelCheckinAndCheckout div에 표시
				$('.adultAmount').eq(0).text(date3);
				$('.childAmount').eq(1).text(date4);
			},
			error: function() {
				console.log("에러 발생");
