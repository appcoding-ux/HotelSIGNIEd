	...
	<div id="wrap">
		...
		<div class="roomList">
			<form action="HotelServlet" name="frm" method="post" id="roomFrm">
				<input type="hidden" name="command" value="Reservation_save" /> <input
					type="hidden" name="bak" value="${bak}" /> <input type="hidden"
					name="originCheckIn" value="${originCheckIn}" /> <input
					type="hidden" name="originCheckout" value="${originCheckOut}" /> <input
					type="hidden" name="checkIn" value="${checkIn}" /> <input
					type="hidden" name="checkOut" value="${checkOut}" /> <input
					type="hidden" name="adultAmount" value="${adult}" /> <input
					type="hidden" name="childAmount" value="${child}" />
				...
				<c:forEach var="room" items="${roomList }">
					...
					<div class="record">
						...
						<div class="room_info">
							<div class="roomName">
								<a href="HotelServlet?command=room_list&num=${room.roomNum}">
									${room.roomName} </a>
							</div>
							<div class="rommViewAndPerson">
								<span>전망 ${room.viewType}</span>
								<span>최대인원수 ${room.roomCapacity}명</span>
								<span>침대타입 ${room.roomType} <img src="image/bed.png" alt="침대사진" /></span>
							</div>
							<div class="roomPrice">
								<span><fmt:formatNumber type="currency"
										value="${room.roomPrice}" currencySymbol="￦"/> KRW</span>
							</div>
							<button type="submit" form="roomFrm" onclick="submitForm()">예약하기</button>
						</div>
					</div>
				</c:forEach>
	...
	<div class="background">
		<div class="detailSearch">
			...
			<form action="HotelServlet" method="post" name="frm" id="sortFrm">
				<input type="hidden" name="command" value="detail_search" class="" />
